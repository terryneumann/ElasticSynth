  ...
  public = list(
    ...
    cv_treated = function() {
      ...
      for (m in 1:length(self$measure_vars)) {
        ...
        treated_wide_measure = self$long_to_wide(self$treated_units[get(eval(self$measure_col)) == self$measure_vars[m], ],
                                            self$time_col,
                                            self$unit_col,
                                            self$value_col)
        donor_wide_measure = self$long_to_wide(self$donor_units[get(eval(self$measure_col)) == self$measure_vars[m], ],
                                          self$time_col,
                                          self$unit_col,
                                          self$value_col)
    ...
    fit = function(weights, cv_results) {
      ...
      for (j in 1:length(self$measure_vars)) {
        ...
        treated_units_wide = self$long_to_wide(self$treated_units[get(eval(self$measure_col)) == self$measure_vars[j],], 
                                          self$time_col, 
                                          self$unit_col, 
                                          self$value_col)
        donor_units_wide = self$long_to_wide(self$donor_units[get(eval(self$measure_col)) == self$measure_vars[j],], 
                                        self$time_col, 
                                        self$unit_col, 
                                        self$value_col)
