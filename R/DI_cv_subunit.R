...
DI_cv_subunit = function(
  sub_units,
  placebo_units,
  treated = 1, 
  pre, 
  post,   
  alphas = seq(0.25, 0.95, by = 0.05),
  lambdas = c(seq(7, 3, by = -.5), seq(2.8, 0.02, by = -0.02), seq(0.01, 0.0001, by = -.0005), 0.0001, 0.00005),
  start_cv,
  end_cv,
  cv_step,
  ...
  verbose = T) 
{
  ...
  na  = length(alphas)
  results = data.frame(alpha = 0, lambda = 0, cv_mse = 0)
  
  cat('\n\n\n\n******* Cross Validation *******\n\n\n\n')
  fold_grid = data.frame()
  for (r in 1:number_folds) {
    cat(paste('***************** fold ', r))
    unit_grid = data.frame()
    for (i in 1:N) {
      Y1       = as.matrix(Y[,i, drop = F])
      unitName = sub('District ', '', names(OutcomeMatrix)[i])
      beats_minus_family = sub_units[,grepl(substr(unitName, 1, 2), substr(names(sub_units), 6, 7)) == F,drop=F]
      
      Z1       = as.matrix(Z[,i, drop = F])
      Z0       = as.matrix(beats_minus_family)   
      ...
      for (j in 1:na) {
        ...
        alpha_grid_tmp = data.frame(unit = colnames(Z1_train), fold = rep(r, length(lambdas)), alpha = rep(a, length(lambdas)), lambda = lambdas, mse = mse_lambda)
        alpha_grid = rbind(alpha_grid, alpha_grid_tmp)
      }
      unit_grid = rbind(unit_grid, alpha_grid)
    }
    fold_grid = rbind(unit_grid, fold_grid)
  }
  final_grid = dcast(fold_grid, lambda + alpha ~ fold + unit, value.var = 'mse')
  row_means  = apply(final_grid[,3:ncol(final_grid)],1,mean)
  best_ix    = which(row_means == min(row_means))
  results$alpha = final_grid[best_ix,'alpha']
  results$lambda = final_grid[best_ix,'lambda']
  results$cv_mse = row_means[best_ix]
